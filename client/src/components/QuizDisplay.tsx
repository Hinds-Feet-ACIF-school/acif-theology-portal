    ...
    // Progress import removed as per earlier request to remove progress display
    ...
    const QuizDisplay: React.FC<QuizDisplayProps> = ({
        ...
        const prevQuizDataIdRef = useRef<string | undefined>();
        ...
        const startQuiz = useCallback(() => {
            ...
            setQuizState('taking');
        }, [isTimed, quizData.settings?.timeLimit, databaseQuizId, setQuizState, setError, setTimeLeft, isContextReady, quizData.id]); // Added quizData.id for consistency
        ...
        const resetQuizStateForRetake = useCallback(() => {
            ...
            setShuffledQuestions, setQuizState, isContextReady, quizData.id // Added quizData.id for consistency
        ]);
        ...
        return (
            <Card className={cn(`not-prose my-4 border ${themedInputBorder}`)}>
                ...
                <CardContent className="pt-5">
                    {error && (quizState !== 'loading_submission' && quizState !== 'initializing_context') && ( <Alert variant="destructive" className="mb-4"> <XCircle className="h-4 w-4" /> <AlertTitle>Error</AlertTitle> <AlertDescription>{error}</AlertDescription> </Alert> )}
                    ...
                    {(!isContextReady && quizState !== 'loading_submission' && quizState !== 'initializing_context') && (
